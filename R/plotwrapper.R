...
plotwrapper <- function(type=c("smoothplot", "highlowplot", "areaplot"), ticker="GOOG", from="2013-01-01", to=Sys.time(), current=FALSE, moyenne=FALSE){
	...
	if(isTRUE(current)){
		...
		myplot <- myplot + geom_text(x = -Inf, y = moyenne, label = paste("Moyenne en devise locale:", moyenne,"; Variance:", var,"; Skewness:", skewness,"; Kurtosis:", kurtosis), hjust = -1, vjust = -0.5, color=blue");	
	}
