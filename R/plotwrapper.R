...
plotwrapper <- function(type=c("smoothplot", "highlowplot", "areaplot"), ticker="GOOG", from="2013-01-01", to=Sys.time(), current=FALSE, moyenne=FALSE){
	...
	if(isTRUE(current)){
		...
		myplot <- myplot + geom_text( y = moyenne, size=5, label = paste("Moyenne en devise locale:", moyenne,"; Variance:", var), hjust = -1, vjust = -0.5, color="blue");	
		myplot <- myplot + geom_text( y = (moyenne/0.9), size=5, label = paste(" Skewness:", skewness,"; Kurtosis:", kurtosis), hjust = -1, vjust = -0.5, color="blue");
	}
